...
def main(min, max, step, fname):
    ...
    for f in np.arange(min, max, step):
        ...
        #Let things stabilize, with at least enough time to get a full sweep
        time.sleep(np.max([0.05, 5/f]))
    ...
    utils.save(settings.DATA_DIR  / "{}".format(fname), np.array(frames), capture_metadata)
